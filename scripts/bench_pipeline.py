...
SYSTEM = (
    "You are Dex, a car mechanic talking to a driver over voice. Speak in short plain sentences. "
        ...
        rows.append(
            {
                ...
                "tts_ms": round(tts_s * 1000),
                "total_ms": round(total),
            }
        )
        print(
            f"\n  ? {q}\n  > {sentence}\n    llm {llm_s * 1000:.0f} ms + tts {tts_s * 1000:.0f} ms "
            f"= {total:.0f} ms to first audio",
            flush=True,
        )

    summary = {
        "model": args.model,
        "vram_llm_mib": llm_only,
        "vram_both_mib": both,
        "vram_peak_mib": peak,
        "headroom_mib": 16376 - peak,
        "turns": rows,
