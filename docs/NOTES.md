...
Осталось нестабильным: одна реплика из шести срывается, причём каждый раз разная — похоже на
гонку в самом тестовом клиенте (чередование отправки тишины и приёма), а не на сервере.
